...
void cal_max() {
    ...
    #pragma omp parallel
        {   
            // Getting thread number
            int tid = omp_get_thread_num();
            int local_max = 0; // Use local max instead of the global max to avoid resource competition

            // Find local max in each thread's workload
            for (int i = tid * workload; i < (tid + 1) * workload; ++i) {
                if (data[i] > local_max) {
                    local_max = data[i];
                }
            ...
            // Update the global max using critical section
    #pragma omp critical
            {
                if (local_max > max) {
                    max = local_max;
